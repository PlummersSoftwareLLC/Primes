...
// Utility functions
// ---------------------------------------------------------------------------
pure nothrow @nogc @safe size_t squareStart(size_t skip) {
    return skip * skip / 2;
}

pure nothrow @nogc @safe size_t minimumStart(size_t skip) {
    return skip / 2 + skip;
}
...
struct PrimeValidator {
    static immutable size_t[8] limits = [
        10, 100, 1000, 10_000, 100_000, 1_000_000, 10_000_000, 100_000_000
    ];
    static immutable size_t[8] counts = [
...
struct ByteStorage {
    ...
    @nogc nothrow @trusted
    void moveFrom(ref ByteStorage rhs) {
        release();
...
struct BitStorage {
    ...
    @nogc nothrow @trusted
    void moveFrom(ref BitStorage rhs) {
        release();

        words = rhs.words;
        lengthBits = rhs.lengthBits;
...
struct BitStorageRotate {
    ...
    @nogc nothrow @trusted
    void moveFrom(ref BitStorageRotate rhs) {
        release();
...
struct StripedBlocks(size_t BLOCK_SIZE, bool HYBRID) {
    ...
    @nogc nothrow @trusted
    void moveFrom(ref StripedBlocks rhs) {
        release();
    ...
    void resetGeneral(size_t skip) @nogc nothrow @trusted {
        ...
        for (auto blockIdx = blockIdxStart; blockIdx < numBlocks; blockIdx++) {
            ...
            while (bitIdx < U8_BITS) {
                ...
                auto endUnrolled = (wordIdx + skip * 3 <= effectiveLen) ? effectiveLen - skip * 3
                    : 0;
...
struct UnrolledHybrid {
    ...
    @nogc nothrow @trusted
    void moveFrom(ref UnrolledHybrid rhs) {
        release();
...
struct ExtremeHybrid {
    ...
    @nogc nothrow @trusted
    void moveFrom(ref ExtremeHybrid rhs) {
        release();
...
struct PrimeSieve(T) {
    ...
    @nogc nothrow @trusted
    void moveFrom(ref PrimeSieve rhs) {
        flags.moveFrom(rhs.flags);
        sieveSize = rhs.sieveSize;
...
void runImplementationST(T)(string label, size_t bitsPerPrime,
    ...
    while (true) {
        ...
        lastSieve.flags.release();
        auto sieve = PrimeSieve!T.create(limit);
        sieve.runSieve();
        lastSieve.moveFrom(sieve);
        localPasses++;
    }
...
auto makeThreadFunc(T)(ThreadResult* result, MonoTime startTime,
...
void runImplementationMT(T)(string label, size_t bitsPerPrime,
    ...
    foreach (i; 0 .. numThreads) {
        threads[i] = new Thread(makeThreadFunc!T(&results[i], startTime, runDuration, limit));
    }
...
int main(string[] args) {
    ...
    auto threadOptions = (opt.threads > 0)
        ? [opt.threads] : getAutoThreadsList(8);
    ...
    foreach (threads; threadOptions) {
        ...
        if (opt.bitsStriped)
            runImplementation!(StripedBlocks!(BLOCK_SIZE_DEFAULT, false))("bit-striped", 1, runDuration, threads, limit, opt
                    .print, repetitions);

        if (opt.bitsStripedBlocks) {
            runImplementation!(StripedBlocks!(BLOCK_SIZE_DEFAULT, false))("bit-striped-blocks16k", 1, runDuration, threads, limit, opt
                    .print, repetitions);
            runImplementation!(StripedBlocks!(BLOCK_SIZE_SMALL, false))("bit-striped-blocks4k", 1, runDuration, threads, limit, opt
                    .print, repetitions);
        }

        if (opt.bitsStripedHybrid) {
            runImplementation!(StripedBlocks!(BLOCK_SIZE_DEFAULT, true))("bit-striped-hybrid-blocks16k", 1, runDuration, threads, limit, opt
                    .print, repetitions);
            runImplementation!(StripedBlocks!(BLOCK_SIZE_SMALL, true))("bit-striped-hybrid-blocks4k", 1, runDuration, threads, limit, opt
                    .print, repetitions);
        }
...
version (unittest) {
    ...
    unittest {
        testStorage!ByteStorage();
    }

    unittest {
        testStorage!BitStorage();
    }

    unittest {
        testStorage!BitStorageRotate();
    }

    unittest {
        testStorage!(StripedBlocks!(BLOCK_SIZE_DEFAULT, false))();
    }

    unittest {
        testStorage!(StripedBlocks!(BLOCK_SIZE_SMALL, false))();
    }

    unittest {
        testStorage!(StripedBlocks!(BLOCK_SIZE_DEFAULT, true))();
    }

    unittest {
        testStorage!(StripedBlocks!(BLOCK_SIZE_SMALL, true))();
    }

    unittest {
        testStorage!(StripedBlocks!(7, false))();
    }

    unittest {
        testStorage!(StripedBlocks!(1024, false))();
    }

    unittest {
        testStorage!(StripedBlocks!(7, true))();
    }

    unittest {
        testStorage!(StripedBlocks!(1024, true))();
    }

    unittest {
        testStorage!UnrolledHybrid();
    }

    unittest {
        testStorage!ExtremeHybrid();
    }
    ...
    unittest {
        sieveKnownCorrect!ByteStorage();
    }

    unittest {
        sieveKnownCorrect!BitStorage();
    }

    unittest {
        sieveKnownCorrect!BitStorageRotate();
    }

    unittest {
        sieveKnownCorrect!(StripedBlocks!(BLOCK_SIZE_DEFAULT, false))();
    }

    unittest {
        sieveKnownCorrect!(StripedBlocks!(BLOCK_SIZE_SMALL, false))();
    }

    unittest {
        sieveKnownCorrect!(StripedBlocks!(BLOCK_SIZE_DEFAULT, true))();
    }

    unittest {
        sieveKnownCorrect!(StripedBlocks!(BLOCK_SIZE_SMALL, true))();
    }

    unittest {
        sieveKnownCorrect!UnrolledHybrid();
    }

    unittest {
        sieveKnownCorrect!ExtremeHybrid();
    }
}
