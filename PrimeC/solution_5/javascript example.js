...
class PrimeSieve {
	...
	runSieve() {
		...
		let q = Math.ceil(Math.sqrt(this.sieveSizeInBits));
		...
		while (factor <= q) {
			...
			console.log('Factor',factor,'start',start,'step',step,'range',range);
			...
			if (range < this.sieveSizeInBits) { // check if we should copy previous results
				...
				console.log('Primes after copyblocks', this.countPrimes(),'source_start',blocksize_bits);
				blocksize_bits = blocksize_bits * step;
			}
			this.bitArray.setBitsTrue(start, step, range);
			console.log('Primes after setbitstrue', this.countPrimes(),'source_start',blocksize_bits);
			factor = this.bitArray.searchBitFalse(factor + 1);

			if (this.bitArray.testBitTrue(194)) console.log('194 is true');
		}
		return this;
	}
