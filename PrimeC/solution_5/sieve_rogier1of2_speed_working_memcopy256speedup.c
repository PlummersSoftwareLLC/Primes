...
static inline struct sieve_state *run_sieve(counter_t maxints) {
    ...
    while (factor <= sieveSizeInBits) {
        ...
        if (range < sieveSizeInBits) {
            ...
            const counter_t blocksize_word = blocksize_bits >> 5;
            const counter_t range_word = range >> 5; // word = 32 bit (2ˆ5)
            const counter_t range_word256 = range_word - blocksize_word;
            for (counter_t blockpart=0; blockpart < blocksize_word; blockpart++) {
                counter_t block_offset_word = blocksize_word + blockpart;

                if ((0==(block_offset_word & 7)) && (blocksize_word - blockpart >= 8) ) {  // byte aligned for mmx256 (32 byte) if word (4 bytes) * 8
                    void *src = &bitarray[block_offset_word];
                    debug printf("reading %d  %d  %d\n", block_offset_word, block_offset_word*32);
                    __m256i tmp0 = _mm256_load_si256((const void *)src);
                    while (block_offset_word < range_word256) {
                        block_offset_word += blocksize_word;
                        debug printf("writing %d  %d\n", block_offset_word, block_offset_word*32);
                        void *dst = &bitarray[block_offset_word];
                        _mm256_stream_si256(dst, tmp0);
                    }
                }

                const bitword_t original = bitarray[block_offset_word];
                while (block_offset_word < range_word) {
                    block_offset_word += blocksize_word;
                    bitarray[block_offset_word] = original;
                }
/*
