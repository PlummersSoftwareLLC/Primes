...
static inline void repeat_words_2_max (
    ...
    while ( (start_at + size ) < max_word ) {
        // unsigned int copy_word = start_at;
        // unsigned int source_word = 1;
        // while (copy_word < start_at+size) {
        //     sieve_state->bit_array[copy_word] = word_values[source_word];
        //     source_word++;
        //     copy_word++;
        // }
...
void set_word_block_size(const unsigned int limit) {
    ...
    for (unsigned int i = 0; i <nr_of_samples; i ++) {
        ...
        printf("Blocksize %d speed %f\n",BLOCK_SIZE*32,speed[i]);
    }
