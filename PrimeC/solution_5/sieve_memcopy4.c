...
#define PATTERN_FASTER (WORD_SIZE*3/4)
#define bitindex(index) ((index)&(WORD_SIZE-1))
#define wordindex(index) ((index) >> SHIFT)
#define  markmask(index) ((bitword_t)1U << bitindex(index))
#define chopmask(index) (((bitword_t)1U << bitindex(index))-(bitword_t)1U)
#define chopmask2(index) (((bitword_t)2U << bitindex(index))-(bitword_t)1U)
#define offsetmask(index) ((index) & ((bitword_t)sizeof(bitword_t)*8-1))
...
//    if (step & 1 == 0) printf("improvement for step %ld possible!\n",step);
	...
	bitword_t pattern = (bitword_t)1U;
    ...
    // bitword_t pattern = patterns[step]; //alternative for above code; but doesn't look faster
...
//       sieve->bitarray[copy_word] |= ((pattern << shift) & (2 << (bitindex(range_stop))) - 1 );
//      printf("Returning at setting first word %ld shift %ld ramge_stop_bit %ld pattern %ld\n", (uint64_t)range_start, (uint64_t)shift, (uint64_t) bitindex(range_stop),(uint64_t)(((uint64_t)1U << 50)) );
       return;
    }
    ...
    // handle laste word with care and ignore the remaining bits
...
static inline void copyPattern(struct sieve_state *sieve, counter_t source_start, counter_t size, counter_t destination_stop)	{
    ...
    if (shift > 0) {
        ...
        bitword_t dest_wordValue = (sieve->bitarray[source_word] >> shift) | (sieve->bitarray[source_word+1] << shift_flipped);
        dest_wordValue &= ~chopmask(copy_bit); // because this is the first word, dont copy the extra bits in front of the source
    ...
    if (shift < 0) {
        ...
        bitword_t dest_wordValue = (sieve->bitarray[source_word] << shift) | (sieve->bitarray[source_lastword] >> shift_flipped);
        dest_wordValue &= ~chopmask(copy_bit); // because this is the first word, dont copy the extra bits in front of the source
        sieve->bitarray[copy_word] |= dest_wordValue; // or the start in to not lose data
        copy_word++;
        source_word++;
        ...
        // printf("After firstword copy_word %ld, source_word-1 %ld, source_word %ld source_lastword %ld copy_bit %ld\n",copy_word,source_word,source_lastword,copy_bit);
        // dump_bitarray(sieve);
...
static inline void sieve_block(struct sieve_state *sieve, counter_t block_start, counter_t block_stop) {
    ...
    do {
        ...
        // printf("Before %ld start %ld\n",(uint64_t)prime, (uint64_t)start);
        // dump_bitarray(sieve);
        setBitsTrue(sieve, start, step, range_stop);
        // printf("After %ld start %ld\n",(uint64_t)prime, (uint64_t)start);
        // dump_bitarray(sieve);
