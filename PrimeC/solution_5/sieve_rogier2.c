...
void run_sieve(struct sieve_state sieve_state) {
  counter_t maxints=sieve_state.maxints;
  TYPE *bitarray=sieve_state.a;
  counter_t maxintsh=maxints>>1U;
  counter_t maxWordOffset = maxintsh >> 5U;
  counter_t qh=(counter_t)sqrt(maxintsh)+1U;
...
//  counter_t step;
  ...
  while( factor<=qh ) {
    counter_t step=(factor<<1U)+1U;
    counter_t start = factor * factor * 2 + 2 * factor;
    // Mask all integer multiples of this prime

    for (i=start; i<maxintsh; i+=step)
      bitarray[i>>SHIFT]|=(TYPE)1<<(i&MASK);

    int wordOffset = factor >> 5;
    int bitOffset = factor & 31;

    counter_t factorold = factor;

    factor &= ~31;
    unsigned int current_word = bitarray[wordOffset];
    current_word >>= bitOffset;
    int jump;
    do {
        unsigned int jumpbase = (current_word & 30) + 2;
        jump = __builtin_ctz(jumpbase);
        current_word >>= jump;
        bitOffset += jump;
        if (bitOffset > 31) {
            printf("Bitoffset > 31 at factor %d => %d\n", factorold, factorold*2+1);
            wordOffset++;
            if (wordOffset > maxWordOffset) {
                break;
            }
            current_word = bitarray[wordOffset];
            bitOffset -= 32;
            factor += 32;
        }
//        printf("jump %d ",jump);
    ...
    factor |= bitOffset;
    printf("factor %d => %d, jump %d\n",factor, factor*2+1, jump);
  }
}
