...
void memcpy_avx_async_pf_cpy(void *dst, const void *src, size_t n) {
  // d, s -> 64 byte aligned
  // n -> multiple of 64

    _mm_prefetch(src, _MM_HINT_T0);
    _mm256_stream_si256((void *)dst,  _mm256_load_si256((const void *)src));
    _mm256_stream_si256((void *)dst+32,  _mm256_load_si256((const void *)src+32));
//    _mm512_store_si512((__m512i*)d, _mm512_load_si512((__m512i const*)s));
}

static inline void * memcpy_aligned(void * dst, void const * src, size_t len)
{
    unsigned long long       * plDst = (unsigned long long *) dst;
    unsigned long long const * plSrc = (unsigned long long const *) src;
...
//    counter_t primeproduct_bits  = (2*3*5) >> 1;
    ...
    counter_t blockpart_size = 64;
    counter_t blockparts     = blocksize_byte / blockpart_size;
    counter_t blocksize_bit  = blocksize_byte << 3;
    counter_t blocks         = ceiling(bits,blocksize_bit)-1;

    debug printf("primeproduct_bits %d  blockbits %d  blockbytes %d  blockwords %d  blocks %d  bits %d blockparts %d\n",primeproduct_bits, blocksize_bit, blocksize_byte, blocksize_word, blocks, bits,blockparts);
...
//    printf("BLocks %d\n", blocks);
    ...
    for (counter_t blockpart=0; blockpart < blockparts; blockpart++) {
        void *src = &bitarray[blocksize_word+blockpart*blockpart_size/sizeof(bitword_t)];
        __m256i tmp0 = _mm256_load_si256((const void *)src);
        __m256i tmp1 = _mm256_load_si256((const void *)src+32);

        for (counter_t block=2; block < blocks; block++) {
            void *dst = &bitarray[block*blocksize_word+blockpart*blockpart_size/sizeof(bitword_t)];
            debug printf("Copying part %d with %d bytes (%d bits) from block 0 to block %d  -> wordindex %d  byteindex %d  bitindex %d\n",blockpart, blocksize_byte, blocksize_bit, block,block*blocksize_word, (blocksize_byte * block), blocksize_bit * block );
            //printf("Copying part %d %d\n", blockpart, (block*blocksize_word+blockpart*blockpart_size)*32 );
//            memcpy_aligned(&bitarray[block*blocksize_word+blockpart*blockpart_size/sizeof(bitword_t)], &bitarray[blocksize_word+blockpart*blockpart_size/sizeof(bitword_t)],blockpart_size);
            _mm256_stream_si256(dst, tmp0);
            _mm256_stream_si256(dst+32, tmp1);
