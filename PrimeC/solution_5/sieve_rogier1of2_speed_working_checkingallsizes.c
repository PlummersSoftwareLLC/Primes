...
static inline struct sieve *create_sieve(counter_t sieveSize) {
    ...
    const counter_t sieveSizeFullWordsInBytes = (1+(sieveSize >> (bitword_shiftbits+1))) << bitword_shiftbits;
    sieve->sieveSizeInBits = sieveSize >> 1;
...
int main(int argc, char **argv) {
    ...
    for (counter_t sizeIndex = 0; sizeIndex < 9; sizeIndex++) {
        ...
        if (primeCount==validPrimes) {
            ...
            if (default_verbose) printf("...No valid result. Sievesize %d was expected to have %d primes, but algoritm produced %d primes\n",known_PrimeCount[sizeIndex], validPrimes, primeCount);
    }
