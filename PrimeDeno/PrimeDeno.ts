...
class PrimeSieve {
  ...
  constructor(sieveSize: number) {
    ...
    this.bits = new Uint8Array(sieveSize);
  }
  ...
  public async printResults(showResults: boolean, duration: number, passes: number): Promise<void> {
    const te = new TextEncoder()
    if (showResults) { 
      await Deno.stdout.write(te.encode("2, "));
    }
    let count = 1;
    for (let num = 3; num < this.sieveSize; num += 2) {
      if (!this.bits[num]) {
        if (showResults) { 
          await Deno.stdout.write(te.encode(`${num}, `));
        }
        count++;
      }
    }
...
while (true) {
  ...
  if (duration >= 5) {
    await sieve.printResults(false, duration, passes);
    break;
  }
}
