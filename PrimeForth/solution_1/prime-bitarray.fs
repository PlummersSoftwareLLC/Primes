...
: get-bit-addr ( addr idx -- bit-idx )
      8 /                              \ get the byte offset
      + ;

: get-bit ( addr idx -- bit )
      tuck                             \ idx addr idx
      get-bit-addr c@                  \ get the char at that location
      swap 8 mod                       \ bit index
      1 swap lshift                    \ bit mask
      and if 1 else 0 endif ;

: set-bit ( addr idx -- )
      tuck
      get-bit-addr dup c@              \ idx addr byte
      rot 8 mod 1 swap lshift          \ get bit mask
      or                               \ apply bit mask
      swap c!
      ;
...
: clear-bit ( addr idx -- )
      ...
      get-bit-addr dup c@              \ idx addr byte
      rot 8 mod 1 swap lshift          \ get bit mask
      invert and                       \ apply bit mask
      swap c!
      ;

: set-all-bits ( addr len -- )
      7 + 8 /  \ get length in bytes
      0 u+do
            dup i + 255 swap c!
        loop drop ;
...
: print-benchmark-results ( timeout sieve-size -- )
        ...
        1e6 f/
        ." tjol-1bit;" . ." ;" f. ." ;1;algorithm=base,faithful=yes" cr
        ;
