...
int main()
{
	...
	while (std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - tStart).count() < 5)
	{
		sieve->runSieve();
		passes++;
	}
	...
	if (sieve)
	{
		sieve->printResults(true, std::chrono::duration_cast<std::chrono::microseconds>(tD).count() / 1000000, passes);
		delete sieve;
	}
}
