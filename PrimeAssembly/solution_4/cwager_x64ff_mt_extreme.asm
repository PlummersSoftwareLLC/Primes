...
; The dense helpers below intentionally compose their multiple-marking pattern
; in executable code from the individual bit positions within one full odd-skip
; period. Each `btr` names a specific multiple bit derived from the current
; skip, so the source shows the base-algorithm marking work directly instead of
; relying on opaque precomputed bitmap constants.
%macro emit_dense_btr_word 2
    mov rax, qword [rsi + %2*8]
    %assign __dense_bit (%1 / 2)
    %rep 64
        %if ((__dense_bit >> 6) = %2)
            btr rax, (__dense_bit & 63)
        %endif
        %assign __dense_bit (__dense_bit + %1)
    %endrep
...
%%loop:
    ...
    %rep %1
        emit_dense_btr_word %1, __dense_word
