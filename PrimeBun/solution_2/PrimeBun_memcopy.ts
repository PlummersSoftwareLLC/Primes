 ...
 *      - new time scale factor as Bun's performance.now() is in nanoseconds, not milliseconds
...
class bitArray
{
    ...
    copyBlocks(blocksize_bits, range)
    {
        ...
        {// reordered loops to increase cpu level1 cahce usage
...
class PrimeSieve
{
    ...
    private sieveSizeInBits : number;
...
const runSieveBatch = (sieveSize, timeLimitSeconds = 5) =>
{
    ...
    const timeFinish = timeStart + timeLimitSeconds * NANOSECONDS_PER_SECOND;  // Calculate finish time before, so we don't repeat this calculation
    ...
    return {
        "durationInSec": (timeEnd - timeStart) / NANOSECONDS_PER_SECOND,
        "totalPasses": totalPasses
    }
}

// get a single sieve (for validation and statistics)
const validateSieveAlgoritm = (sieveSize, maxShowPrimes = 100) =>
{
...
const main = (config : IConfig) =>
{
    ...
    const validationResult = validateSieveAlgoritm(sieveSize);
    ...
    if (validationResult.validResult === true)
    {
        ...
        const { totalPasses, durationInSec } = batchResult;

        //output for benchmarking purposes
        const label = process.argv[1].endsWith(".js")
            ? "\ndiefbell_tsc_memcopy"
            : "\ndiefbell_ts-bun_memcopy";
        const msg = [
            label,  // label
