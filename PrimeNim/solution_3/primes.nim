...
func `{}`(a: BitList, i: SomeInteger): bool {.inline.} =
  let 
    byteIndex = i div 8
    bitInd = i mod 8
  a[byteIndex].testBit(bitInd)

func `{}=`(a: var BitList, i: SomeInteger, val: bool) {.inline.}=
  let 
    byteIndex = i div 8
    bitInd = i mod 8
  if val:
...
func countPrimes(this: PrimeSieve): uint64 =
    var count = 1'u64
...
proc printResults(this: PrimeSieve, showResults: bool, duration: float64, passes: int) =
    if showResults:
        stdout.write("2, ")
...
func newPrimeSieve(sieveSize: uint64): PrimeSieve =
    PrimeSieve(sieveSize: sieveSize, bits: newSeq[uint64](sieveSize div 8))
