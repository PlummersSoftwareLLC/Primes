...
public class PrimeSieveJava
{
	...
	public void printResults(double duration, int passes)
	{
		...
		System.out.printf("Passes: %d, Time: %f, Avg: %f, Limit: %d, Count: %d, Valid: %s%n", passes,
				duration, (duration / passes), sieveSize, count, validateResults());
		System.out.printf("MansenC+pez;%d;%f;1;algorithm=base,faithful=yes,bits=8\n", passes, duration);
	}
	...
	public static void main(String[] args)
	{
		...
		if (warmup)
		{
			...
			{
				sieve = new PrimeSieveJava(limit);
				sieve.run();
			}
		}
