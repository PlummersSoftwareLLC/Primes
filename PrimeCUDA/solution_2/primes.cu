...
__global__ void unmark_multiples(uint32_t primeCount, uint32_t *primes, uint64_t halfSize, uint32_t sizeSqrt, uint32_t *sieve)
{
...
class Sieve 
{
    ...
    uint32_t *run()
    {
        ...
        // Unmark multiples on the GPU and release the prime list buffer
        unmark_multiples<<<1,THREAD_COUNT>>>(primeCount, devicePrimeList, half_size, size_sqrt, device_sieve_buffer);
...
void printResults(uint64_t sieveSize, uint64_t primeCount, double duration, uint64_t passes)
{
    ...
    printf("rbergen_faithful_cuda;%zu;%f;1;algorithm=base,faithful=yes,bits=1\n", passes, duration);
}
