...
(let* ((passes 0)
  ...
  (let* ((duration  (/ (- (get-internal-real-time) start) internal-time-units-per-second))
    ...
    (format *error-output* "Algorithm: base w/ modulo functions  Threads: ~d  Passes: ~d  Time: ~f Avg: ~f ms Count: ~d  Valid: ~A~%"
            +threads+ passes duration (* 1000 avg) (count-primes result) (validate result))

    (format t "mayerrobert-YaroslavKhnygin-cl-modulo-functions;~d;~f;~d;algorithm=base,faithful=yes,bits=1~%" passes duration +threads+)))
