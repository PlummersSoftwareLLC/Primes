...
pub mod primes {
    ...
    pub struct PrimeValidator(&'static [(usize, usize)]);
    ...
    impl PrimeValidator {
        ...
        pub fn is_valid(&self, sieve_size: usize, result: usize) -> Option<bool> {
            if let Some((_size, expected)) = self.0.iter().find(|i| i.0 == sieve_size) {
    ...
    pub const VALIDATOR: PrimeValidator = PrimeValidator::new();
...
mod tests {
    ...
    fn sieve_known_correct<T: FlagStorage>() {
        for (sieve_size, expected_primes) in primes::VALIDATOR.known_results_iter() {
